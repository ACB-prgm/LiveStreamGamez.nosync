...
func batch_put_item(table_name: String, items: Array, add_headers: Dictionary = {}) -> Array:
	if requirements_met(true):
		...
		# Prepare the RequestItems structure for batch operations
		var request_items = {}
		...
		# The body now contains RequestItems with the batch of PutRequest items
		var body = to_json({
			"RequestItems": request_items
		})
		# Update the x-amz-target to 'BatchWriteItem' for batch operations
		add_headers.merge({
